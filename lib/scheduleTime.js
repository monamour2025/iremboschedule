...
export function liveIremboRowMatchesCategory(row, licenseCategory) {
  const want = String(licenseCategory || "").trim().toUpperCase();
  if (!want) {
    return false;
  }
  return scheduleMatchesCategory(
    {
      category: row?.categoryOrLane || row?.category || row?.licenseCategory,
      scheduleId: row?.scheduleId || row?.id || row?.examScheduleId
    },
    want
  );
}
