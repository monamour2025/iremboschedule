...
export function isUpcomingSchedule(schedule, now = new Date()) {
...
export function isOpenUpcomingSchedule(schedule, now = new Date()) {
  return Number(schedule?.remainingCapacity || 0) > 0 && isUpcomingSchedule(schedule, now);
}

export function examSlotIdentity(schedule) {
  const start = schedule?.startDateTime ? new Date(schedule.startDateTime) : null;
  const startKey = start && !Number.isNaN(start.getTime()) ? start.toISOString() : "";
  return [
...
export function dedupeOpenUpcomingSchedules(schedules = [], now = new Date()) {
  const bySlot = new Map();
  for (const schedule of schedules) {
    if (!isOpenUpcomingSchedule(schedule, now)) {
      continue;
    }
    const key = examSlotIdentity(schedule);
    const existing = bySlot.get(key);
    if (!existing || Number(schedule.remainingCapacity || 0) > Number(existing.remainingCapacity || 0)) {
      bySlot.set(key, schedule);
    }
  }
...
export function extractLicenseCategoryToken(value) {
  const raw = String(value || "").trim().toUpperCase();
  if (!raw) {
    return "";
  }
  if (/^[A-E]1?$/.test(raw) || raw === "F") {
    return raw;
  }
  const labeled = raw.match(/\b(?:CATEGORY|CAT|CLASS|LANE)[\s:_-]*([A-E]1?)\b/);
  if (labeled) {
    return labeled[1];
  }
  const start = raw.match(/^([A-E]1?)(?:[\s\-_/: ]|$)/);
  return start ? start[1] : "";
}

export function resolveScheduleCategory(schedule) {
  const fromField = extractLicenseCategoryToken(schedule?.category);
  if (fromField) {
    return fromField;
  }
  return extractLicenseCategoryToken(String(schedule?.scheduleId || "").split(":")[0]);
}

export function scheduleMatchesCategory(schedule, category) {
  const want = extractLicenseCategoryToken(category);
  const got = resolveScheduleCategory(schedule);
  return Boolean(want && got && want === got);
}

export function liveRowRemainingCapacity(row) {
  if (!row || typeof row !== "object") {
    return null;
  }
  const raw = row.remainingCapacity ?? row.remainingSlots ?? row.availableSlots ?? row.availablePlaces;
...
export function liveRowHasOpenSeats(row) {
  const remaining = liveRowRemainingCapacity(row);
  if (remaining === null) {
    return true;
  }
  return remaining > 0;
...
export function liveIremboRowMatchesCategory(row, licenseCategory) {
  const want = extractLicenseCategoryToken(licenseCategory);
  if (!want) {
    return false;
  }
  const got = extractLicenseCategoryToken(
    row?.categoryOrLane || row?.category || row?.licenseCategory
  );
  if (!got) {
    return true;
  }
...
export function timeMatchesRequestedSlot(requestedTime, rangeStart, rangeEnd, candidateTime) {
  ...
  const wanted = normalizeExamTimeInput(requestedTime);
  const got = normalizeExamTimeInput(candidateTime);
