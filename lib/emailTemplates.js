...
export function buildApplicationCreatedEmail({
  fullName,
  applicationNumber,
  status,
  examCenter,
  ...
  const siteLine = [examCenter, location].filter(Boolean).join(" · ") || "Not specified";
  const slotLine = [examDate, examTime].filter(Boolean).join(" ") || "Not specified";
