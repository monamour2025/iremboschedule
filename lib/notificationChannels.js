...
import { getOfficeNotificationEmail, resolveNotificationEmail } from "./notificationInbox.js";

export function resolveTargets(targets = {}) {
  return {
    email: resolveNotificationEmail(targets.email || process.env.ALERT_EMAIL),
...
function parseResendAllowedRecipient(message) {
  const match = String(message || "").match(/your own email address \(([^)]+)\)/i);
  return match ? String(match[1] || "").trim() : "";
}

async function postResendEmail({ to, subject, text, html, resendKey }) {
  await axios.post(
    "https://api.resend.com/emails",
    {
      from: process.env.RESEND_FROM || "Irembo Monitor <onboarding@resend.dev>",
      to: [to],
      subject,
      text,
      html
...
async function sendEmail({ title, message, schedule, targets, type, emailMeta }) {
  const resolved = resolveTargets(targets);
  const to = resolved.email || getOfficeNotificationEmail();
  if (!to) {
  ...
  if (resendKey) {
    ...
    try {
      await postResendEmail({ to, subject, text, html, resendKey });
      return { status: "SENT" };
    } catch (error) {
      const apiMessage =
        ...
        (Array.isArray(error?.response?.data?.errors)
          ...
          : null);
      const allowed = parseResendAllowedRecipient(apiMessage || error.message);
      if (allowed && allowed.toLowerCase() !== to.toLowerCase()) {
        const fallbackSubject = `${subject} · for ${to}`;
        const fallbackText = `Intended inbox: ${to}\n\n${text}`;
        const fallbackHtml = `<p><strong>Intended inbox:</strong> ${to}</p>${html}`;
        try {
          ...
          logger.warn("Resend test mode redirected email to the verified account inbox", {
            intended: to,
            deliveredTo: allowed
          });
          return { status: "SENT" };
          ...
          throw new Error(
            fallbackError?.response?.data?.message || fallbackError.message || apiMessage || error.message
          );
        }
      }
